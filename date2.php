...
<p><b>From Date: <?php echo $from_date;?></b></p>
<p><b>To Date: <?php echo $to_date;?></b></p>
...
// $a="SELECT uploadby,phoneno,empid,datecheck,min(time) as time,checkoutdate,max(checkoutime) as checkoutime from request where phoneno in ($emplist) and datecheck between '$from_date' and '$to_date' desc by i ";
// $a="SELECT uploadby,empid,indid FROM request WHERE date BETWEEN from_date AND to_date";
$a="SELECT empid,uploadby,indid,datecheck,sname FROM  request 
						  where datecheck between '$from_date' and '$to_date' ";
                        //   $b = mysqli_num_rows($link,$a);
