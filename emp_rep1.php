...
$objPHPExcel->getActiveSheet()->mergeCells('A1:O1');
 ...
 $objPHPExcel->getActiveSheet()->setCellValue('A1', 'SMTS GROUPS');
...
$objPHPExcel->getActiveSheet()->SetCellValue('A6', 'Emp ID');
$objPHPExcel->getActiveSheet()->SetCellValue('B6', 'Emp Name');
$objPHPExcel->getActiveSheet()->SetCellValue('C6', 'Check-IN Location');
$objPHPExcel->getActiveSheet()->SetCellValue('D6', 'Check-IN Time');
$objPHPExcel->getActiveSheet()->SetCellValue('E6', 'Check-OUT Location');
$objPHPExcel->getActiveSheet()->SetCellValue('F6', 'Check-OUT Time');
$objPHPExcel->getActiveSheet()->SetCellValue('G6', 'Last Updated Location');
$objPHPExcel->getActiveSheet()->SetCellValue('H6', 'Last Updated Time');
$objPHPExcel->getActiveSheet()->SetCellValue('I6', 'Work Done');
...
$objPHPExcel->getActiveSheet()->getStyle("A6:O6")->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID)
        ->getStartColor()->setRGB('1576F7');
    //    $objPHPExcel->getActiveSheet()->getRowDimension('A')->setRowHeight(12);
...
while($row	=	$result->fetch_assoc()){
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('D'.$rowCount, mb_strtoupper($row1['loc'],'UTF-8'));
	$objPHPExcel->getActiveSheet()->SetCellValue('E'.$rowCount, mb_strtoupper($row1['time,date'],'UTF-8'));
	$objPHPExcel->getActiveSheet()->SetCellValue('F'.$rowCount, mb_strtoupper($row1['checkoutloc'],'UTF-8'));
	$objPHPExcel->getActiveSheet()->SetCellValue('G'.$rowCount, mb_strtoupper($row1['checkouttime'],'UTF-8'));
	$objPHPExcel->getActiveSheet()->SetCellValue('H'.$rowCount, mb_strtoupper($row1['lastupdateloc'],'UTF-8'));
	$objPHPExcel->getActiveSheet()->SetCellValue('I'.$rowCount, mb_strtoupper(date('d-m-Y',strtotime($row['inv_date'])),'UTF-8'));
	$objPHPExcel->getActiveSheet()->SetCellValue('J'.$rowCount, mb_strtoupper($row['lastupdatedtime'],'UTF-8'));
	$objPHPExcel->getActiveSheet()->SetCellValue('K'.$rowCount, mb_strtoupper($row['com'],'UTF-8'));
...
header('Content-Disposition: attachment;filename="employeereport.xlsx"'); //tell browser what's the file name
