...
if (isset($_POST["submit"])) {
    ...
    if (in_array($_FILES["file"]["type"], $allowedFileTypes)) {
        ...
        default: break;
            ...
            // Check if a record exists for the given month, year, and empid
            ...
            if ($c > 0) {   
                $existingRow = mysqli_fetch_assoc($p);
                if ($existingRow['name'] !== addslashes($ename)) {
                    // Update only if the name is different
                    $m = "UPDATE salarymonthwise 
                    SET name = '" . addslashes($ename) . "', 
                        basic = '$basic', 
                        others = '$others', 
                        pf = '$pf', 
                        pt = '$pt', 
                        ...
                        in_words = '$in_words' 
                    WHERE email = '$empid' AND month_ = '$month' AND year = '$year'";
                    $conn->query($m);
                }
            } else {
